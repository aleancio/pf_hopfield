...
int main() {
  ...
  std::cout << "---- This is the pattern restoration section. Please follow the instructions below ----\n\n";

  try {
    std::string filePath{"#"};
    std::cout << "For demonstration purposes, the memory file used here is the one generated by ClassicLearn.\n";
    std::cout << "Please enter the absolute path to the file where memory is stored (type '#' to use the default):\t";
    std::cin >> filePath;

    if (filePath == "#") {
      filePath = "ClassicMatrixValues.txt";
    }
    ...
    std::cout << "------------------------------------------------------------------\n";
    std::cout << "Now, you can load a corrupted pattern to recognize and correct it.\n";
    std::cout << "For demonstration purposes, you can use one of the images used during training (type '#' for a test image):\t";
    std::cin >> filePath;
    if (filePath == "#") {
      filePath = "ClassicHopfieldNetwork/images/bacio.jpg";
    }

    unsigned int patternSize{
        static_cast<unsigned int>(std::sqrt(net.getMatrix().size()))};
    ...
    pattern.corrupt(static_cast<size_t>(std::ceil(
        static_cast<double>(net.getMatrix().size()) * 0.35)));  // corrupting
                                                                // 35%
    ...
    std::cout << "pattern has been generated and corrupted by 35%\n";
    ...
    double Energy = net.totalEnergy(pattern.getPattern());
    ...
    while (window.isOpen()) {
      ...
      if (start) {
        if (!net.restorePattern_withAnnealing(pattern.elaboratePattern(), n)) {
          std::cout << '#' << std::flush;
          updateTexture.update(pattern.printPattern());
          updateSprite.setTexture(updateTexture);

          // assert(net.totalEnergy(pattern.getPattern()) <= Energy);
          Entext.setString("Energy: " + std::to_string(net.totalEnergy(
                                            pattern.getPattern())));
          ++n;
        } else {
          start = false;
